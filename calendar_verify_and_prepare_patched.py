...
def iso_week_id(s: pd.Series) -> pd.Series:
    ...
    def f(x):
        ...
        try:
            ...
            dts = pd.to_datetime(x, errors="coerce")
            if pd.isna(dts):
                return None
            d = dts.date() if hasattr(dts, "date") else None
            if d is None:
                return None
        iso = d.isocalendar()  # IsoCalendarDate (py3.8+) or tuple
        y = getattr(iso, "year", iso[0])
        w = getattr(iso, "week", iso[1])
        return f"{y}{w:02d}"
    return s.map(f)
