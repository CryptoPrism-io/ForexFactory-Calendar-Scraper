...
cd /d "%BASE%"
if errorlevel 1 ( echo [err] could not cd to %BASE% & exit /b 1 )
...
echo.
echo [2/6] Prefilter signals (sessions + impact>=2 + |z|>=1.0 + dedupe)...
...
%PY% split_signals.py --in_csv outputs\pair_signals_LN.csv --out_dir outputs ^
  --train_end 2018-12-31 --val_end 2021-12-31 || goto :err
...
echo.
echo [done] All runs complete. See:
...
echo    sessions=%~1  out=%~2
%PY% filter_signals_prejoin.py --pairs_csv outputs\calendar_surprises_pair_signals.csv ^
  --events_csv outputs\calendar_surprises.csv ^
  --out_csv %2 --sessions %~1 --impact_min 2 --zmin 1.0 ^
  --dedupe_policy impact_then_absz --min_gap_min 0
exit /b %errorlevel%
...
%PY% price_join_and_score_v2.py ^
  --pairs_csv "%PAIRS%" ^
  --events_csv outputs\calendar_surprises.csv ^
  --prices_dir "%PRICES_DIR%" ^
  --file_pattern "%PAT%" --tf %TF% ^
  --ts_col datetime --prices_tz UTC ^
  --windows %WIN% --tag %TAG%
exit /b %errorlevel%
