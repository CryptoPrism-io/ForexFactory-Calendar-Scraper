#!/usr/bin/env python3
# See chat for docstring. EODHD adapter.
...
def main():
    ...
    url = f"https://eodhd.com/api/calendar/economic-events?from={args.start}&to={args.end}&api_token={args.token}&fmt=json"
    data = fetch(url)
    if not isinstance(data, list): print("[warn] unexpected response"); return

    rows = []
    for it in data:
        ccy = (it.get("currency") or it.get("country_code") or "").upper()
        impact = (str(it.get("importance") or "")).lower()
        impact_num = {"low":1,"medium":2,"high":3}.get(impact, 0)
        title = it.get("event") or it.get("title") or ""
        when_iso = it.get("date") or it.get("datetime") or ""
        ...
        rows.append({
            "event_id": f"eod:{ccy}:{title}:{when_iso}",
    ...
    con.execute("""CREATE TABLE IF NOT EXISTS events_eodhd (
    ...
    for rec in df.to_dict(orient="records"):
        ...
        cur.execute("INSERT OR REPLACE INTO events_eodhd ("+cols+") VALUES ("+ph+")", tuple(rec.values()))
    con.commit(); con.close()
    print(f"[upserted] {len(df)} rows into {args.db}::events_eodhd")
