#!/usr/bin/env python3
"""
Database utilities for ForexFactory automation
Handles PostgreSQL connections, queries, and data operations
"""
...
class DatabaseManager:
    ...
    # ===== FOREX_EVENTS TABLE OPERATIONS =====

    def insert_events(self, events_list, source="unknown"):
        """
        Insert events into Economic_Calendar_FF table, skip duplicates
            ...
            with self.get_connection() as conn:
                with conn.cursor() as cursor:
                    for event in events_list:
                        ...
                        event = {k: (v if v and v.strip() else None) for k, v in event.items()}
    ...
    def update_actual_values(self, updates_list):
        """
        Update actual values for events

        Args:
            updates_list: List of dicts with keys: date, currency, event, actual
        ...
        query = """
            ...
            SET actual = %(actual)s, updated_at = CURRENT_TIMESTAMP
            WHERE date = %(date)s AND currency = %(currency)s AND event = %(event)s
    ...
    def get_events_by_currency_and_impact(self, currency=None, impact=None):
        ...
        Returns:
            List of dicts containing event data
        """
        query = "SELECT date, time, currency, impact, event, actual, forecast, previous FROM Economic_Calendar_FF WHERE 1=1"
        params = {}
